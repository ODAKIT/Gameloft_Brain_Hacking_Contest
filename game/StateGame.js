	...
	*/
	constructor()
	{
		...
		*/
		...
		this.scoreText = new PIXI.Text('0/7000', style);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	Load()
	{
		...
		if(this.isLoad==false)
		{
			...
			loader.add('hint','data/image/hint.png');
		...
		//Khởi tạo vận tốc cho mỗi khối
			...
			//load image
			loader.add('blue', 'data/image/blue.png')   //0
			loader.add('green', 'data/image/green.png') // 1
			loader.add('pink', 'data/image/pink.png')   // 2
			loader.add('yellow', 'data/image/yellow.png') // 3
			loader.add('bg', 'data/common/background.jpg')
			loader.add('special','data/image/special.png') //5;
			loader.add('frenzy','data/image/frenzy.png')  //8
			loader.add('timebar','data/image/timebar.jpg')
			loader.add('uibrand','data/image/ui-brand.png')
			loader.add('clock','data/image/clock.png')
			loader.add('scoreUI','data/image/score.png')
			loader.add('frenzyTimeUI','data/image/score.png')
			loader.add('frenzyTimeBar','data/image/frenzybar.png')
			loader.add('hand','data/common/hand.png')
			loader.add('gameover','data/image/gameover.png');
			loader.add('home','data/image/Home.png')
...
//------------------------------------------------------------------CÁC THAO TÁC XỬ LÝ CHÍNH TRONG GAME ----------------------------------------------------------------------------//
	...
	ClickBlock()
	{
		...
		if ((this.targetX != -1 || this.targetY != -1) && this.isNewClick==false) 
		{
			...
			if(this.count+1>=3)
			{
				...
				CreateNewBlock(this.xArray,this.yArray,this.block,this.matrixBlock);
				...
				if(this.count+1>=5 && this.isSpecial==true)
				{
						...
						{
							this.block[this.yClick*7+this.xClick].texture=PIXI.loader.resources.special.texture
							this.matrixBlock[this.yClick][this.xClick]=5;
						}
						else
						{
							this.block[this.yClick*7+this.xClick].texture=PIXI.loader.resources.frenzy.texture
	...
	CheckTimeReturnMenuScene(deltaTime)
	{
		if(this.timeOfGame<=1)
		{
			...
			this.timeSwitch+=deltaTime*1.5;
			APP.alpha-=0.015
			if(this.timeSwitch>4 && this.score>=7000)
			{
				this.gameover.alpha=0;
				this.timeOfGame=30;
				Score = this.score;
			...
			else if(this.timeSwitch>4 && this.score<7000)
			{
				this.gameover.alpha=0;
				this.timeOfGame=30;
				Score = this.score;
					...
					if(this.matrixBlock[i][j]==8)
						{
							this.matrixBlock[i][j]=5;
								...
								this.block[i*7+j].texture = PIXI.loader.resources.special.texture;
					...
					if(this.matrixBlock[i][j]==5)
						{
							this.matrixBlock[i][j]=8;
								...
								this.block[i*7+j].texture = PIXI.loader.resources.frenzy.texture;
							...
							for(let i = 0 ; i<4 ; i++)
								{
									...
									this.block[y*7+x].texture=PIXI.loader.resources.frenzy.texture
	...
	this.scoreText.text=this.score+'/7000';
	}
...
//---------------------------------------------------------------------CÁC PHƯƠNG THỨC LOAD----------------------------------------------------------------------
	...
	LoadProgressHandler(loader, resource)
	{
		switch (resource.name)
		{
			...
			case "timebar":
			{
				this.timeBar = new PIXI.Sprite(PIXI.loader.resources.timebar.texture);
			...
			case "uibrand":
			{
				this.uiBrand = new PIXI.Sprite(PIXI.loader.resources.uibrand.texture);
			...
			case "clock":
			{
				this.clock = new PIXI.Sprite(PIXI.loader.resources.clock.texture);
			...
			case "scoreUI":
			{
				this.scoreUI = new PIXI.Sprite(PIXI.loader.resources.scoreUI.texture);
			...
			case "frenzyTimeUI":
			{
				this.frenzyTimeUI = new PIXI.Sprite(PIXI.loader.resources.frenzyTimeUI.texture);
			...
			case "frenzyTimeBar":
			{
				this.frenzyTimeBar = new PIXI.Sprite(PIXI.loader.resources.frenzyTimeBar.texture);
		...
		if(this.isLoad==false)
		{
			...
			for(let i = 0 ; i<7 ; i++)
			{
				for(let j = 0 ; j<7 ; j++)
				{
					...
					if(x==0)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.blue.texture));
					}
					else if(x==1)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.green.texture));
					}
					else if(x==2)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.yellow.texture));
					}
					else if(x==3)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.pink.texture));
					}
					this.matrixBlock[i][j]=x;

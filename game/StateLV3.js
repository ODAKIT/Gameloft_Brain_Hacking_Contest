...
class StateLV3 extends PIXI.Container
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	Load()
	{
		...
		if(this.isLoad==false)
		{
			...
			loader.add('hintm','data/image/hint.png');
		...
		//Khởi tạo vận tốc cho mỗi khối
			...
			//load image
			loader.add('grey', 'data/image/grey.png')   //0
			loader.add('dark', 'data/image/dark.png') // 1
			loader.add('bgm', 'data/common/background.jpg')
			loader.add('specialm','data/image/special.png') //5;
			loader.add('frenzym','data/image/darkfrenzy.png')  //8
			loader.add('timebarm','data/image/timebar.jpg')
			loader.add('uibrandm','data/image/ui-brand.png')
			loader.add('clockm','data/image/clock.png')
			loader.add('scoreUIm','data/image/score.png')
			loader.add('frenzyTimeUIm','data/image/score.png')
			loader.add('frenzyTimeBarm','data/image/frenzybar.png')
			loader.add('gameoverm','data/image/gameover.png')
			loader.add('gamehomem','data/image/Home.png');
		...
		for(let i = 0 ; i<7 ; i++)
			{
				...
				{	
						...
						if(x==0)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.grey.texture;
						}
						else if(x==1)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.dark.texture;
						}
...
//------------------------------------------------------------------CÁC THAO TÁC XỬ LÝ CHÍNH TRONG GAME ----------------------------------------------------------------------------//
	...
	ClickBlock()
	{
		...
		if ((this.targetX != -1 || this.targetY != -1) && this.isNewClick==false) 
		{
			...
			if(this.count+1>=3)
			{
				...
				CreateNewBlockLV3(this.xArray,this.yArray,this.block,this.matrixBlock);
				...
				if(this.count+1>=5 && this.isSpecial==true)
				{
						...
						{
							this.block[this.yClick*7+this.xClick].texture=PIXI.loader.resources.specialm.texture
							this.matrixBlock[this.yClick][this.xClick]=5;
						}
						else
						{
							this.block[this.yClick*7+this.xClick].texture=PIXI.loader.resources.frenzym.texture
	...
	CheckTimeReturnMenuScene(deltaTime)
	{
		if(this.timeOfGame<=1)
		{
			...
			APP.alpha-=0.015
			if(this.timeSwitch>3 && this.score>20000)
			{
				this.timeOfGame=30;
				Score = this.score;
				this.Reset();
			...
			else if(this.timeSwitch>3 && this.score<20000)
			{
				this.timeOfGame=30;
				Score = this.score;
				this.Reset();
		...
		for(let i = 0 ; i<7 ; i++)
			{
				...
				{	
						...
						if(x==0)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.blue.texture;
						}
						else if(x==1)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.green.texture;
						}
							else if(x==2)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.yellow.texture;
						}
						else if(x==3)
						{
							this.block[i*7+j].texture = PIXI.loader.resources.pink.texture;
						}
						this.matrixBlock[i][j]=x;
					...
					if(this.matrixBlock[i][j]==8)
						{
							this.matrixBlock[i][j]=5;
								...
								this.block[i*7+j].texture = PIXI.loader.resources.specialm.texture;
					...
					if(this.matrixBlock[i][j]==5)
						{
							this.matrixBlock[i][j]=8;
								...
								this.block[i*7+j].texture = PIXI.loader.resources.frenzym.texture;
							...
							for(let i = 0 ; i<4 ; i++)
								{
									...
									this.block[y*7+x].texture=PIXI.loader.resources.frenzym.texture
	...
	this.scoreText.text=this.score + '/20000';
	}
...
//---------------------------------------------------------------------CÁC PHƯƠNG THỨC LOAD----------------------------------------------------------------------
	...
	LoadProgressHandler(loader, resource)
	{
		switch (resource.name)
		{
			...
			case "timebarm":
			{
				this.timeBar = new PIXI.Sprite(PIXI.loader.resources.timebarm.texture);
			...
			case "uibrandm":
			{
				this.uiBrand = new PIXI.Sprite(PIXI.loader.resources.uibrandm.texture);
			...
			case "clockm":
			{
				this.clock = new PIXI.Sprite(PIXI.loader.resources.clockm.texture);
			...
			case "scoreUIm":
			{
				this.scoreUI = new PIXI.Sprite(PIXI.loader.resources.scoreUIm.texture);
			...
			case "frenzyTimeUIm":
			{
				this.frenzyTimeUI = new PIXI.Sprite(PIXI.loader.resources.frenzyTimeUIm.texture);
			...
			case "frenzyTimeBarm":
			{
				this.frenzyTimeBar = new PIXI.Sprite(PIXI.loader.resources.frenzyTimeBarm.texture);
			...
			case "gameoverm":
				{
					this.gameover = new PIXI.Sprite(PIXI.loader.resources.gameoverm.texture)
			...
			case "gamehomem":
				{
					this.home = new PIXI.Sprite(PIXI.loader.resources.gamehomem.texture)
		...
		if(this.isLoad==false)
		{
			...
			for(let i = 0 ; i<7 ; i++)
			{
				for(let j = 0 ; j<7 ; j++)
				{
					...
					if(x==0)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.grey.texture));
					}
					else if(x==1)
					{
						this.block.push(new PIXI.Sprite(PIXI.loader.resources.dark.texture));
					}
					this.matrixBlock[i][j]=x;
					this.blockAnim[i*7+j].position.set(this.curPosX+j*this.Direc,this.curPosY+i*this.Direc);
					
					//Ô nhắc
					this.blockHint.push(new PIXI.Sprite(PIXI.loader.resources.hintm.texture))
